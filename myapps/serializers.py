...
class RegisterSerializer(serializers.ModelSerializer):
    ...
    def validate_email(self, value):
        # Real-time email validation using Mailboxlayer
        api_key = 'd473660f4966b7ae7fdd1c7869d5fab9'  # Replace with your actual key
        url = f"https://apilayer.net/api/check?access_key={api_key}&email={value}&smtp=1&format=1"

        try:
            response = requests.get(url, timeout=5)
            result = response.json()

            if not result.get('smtp_check'):
                raise serializers.ValidationError("This email does not appear to exist.")
        except requests.exceptions.RequestException as e:
            raise serializers.ValidationError(f"Email verification failed: {str(e)}")

        return value

    def validate(self, data):
        # Explicitly call password validator
